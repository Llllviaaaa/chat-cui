...
### Phase 08 Distributed Precise Delivery Closure (Provisional IDs)

- [x] **P08-ROUTE-01**: Redis route table is source of truth keyed by `tenant_id + session_id`, with explicit `route_version` CAS semantics.
- [x] **P08-FENCE-01**: Owner transfer applies immediate fence to stale owner and returns deterministic `OWNER_FENCED` behavior.
- [x] **P08-RELAY-01**: Non-target gateway instances relay through `skill-service owner` before target `gateway/client` delivery.
- [x] **P08-DEDUPE-01**: Cross-hop dedupe key uses `session_id + turn_id + seq + topic` to prevent duplicate writes/delivery.
- [x] **P08-ACK-01**: Delivery lifecycle exposes deterministic two-stage acknowledgment (`gateway_owner_accepted`, `client_delivered`/timeout).
- [x] **P08-RECOVERY-01**: Unknown-owner replay recovery is bounded (15 minutes) and ends in deterministic terminal envelope when exhausted.
- [x] **P08-OBS-01**: Route/fence/relay/ack/recovery outcomes are observable via low-cardinality metrics and traceable structured logs.
...
| P08-ROUTE-01 | Phase 8 | Complete |
| P08-FENCE-01 | Phase 8 | Complete |
| P08-RELAY-01 | Phase 8 | Complete |
| P08-DEDUPE-01 | Phase 8 | Complete |
| P08-ACK-01 | Phase 8 | Complete |
| P08-RECOVERY-01 | Phase 8 | Complete |
| P08-OBS-01 | Phase 8 | Complete |

**Coverage:**
- v1 requirements: 23 total
- Additional milestone provisional IDs: 22 total (`P01.1-*`, `P07-*`, `P08-*`)
- Mapped to phases: 45
- Unmapped: 0

---
*Requirements defined: 2026-03-03*  
*Last updated: 2026-03-04 after executing Phase 8 Plan 08-04*
